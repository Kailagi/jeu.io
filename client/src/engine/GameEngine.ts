...
export class GameEngine {
      ...
      try {
        ...
        switch (data.type) {
          ...
          case 'GAME_STATE':
            ...
            this.remotePlayers = data.players.filter(
              (p: RemotePlayer) => p.id !== this.myNetworkId
            );
  ...
  private gameLoop = (): void => {
    ...
    this.remotePlayers.forEach((p) =>
      this.renderer.drawPlayer(p.x, p.y, p.radius, p.color, p.pseudo)
    );

    let renderX = this.playerX,
      renderY = this.playerY;
  ...
  private drawDustClouds(): void {
    ...
    this.activeClouds.forEach((cloud) => {
      ...
      gradient.addColorStop(
        0,
        cloud.ownerId === this.myNetworkId ? 'rgba(243, 179, 179, 0.4)' : 'rgba(226, 210, 151, 0.4)'
      );
  ...
  private drawUIBar(): void {
    const ctx = this.renderer.ctx;
    const x = this.playerX - 25,
      y = this.playerY + this.playerRadius + 12;
